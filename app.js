...
function bindEvents() {
  ...
  document.querySelectorAll('input[name="meter"]').forEach(inp => {
    inp.addEventListener('change', () => {
      ...
      const hint = document.getElementById('touHintDisplay');
      if (hint) hint.classList.toggle('hidden', state.meter !== 'tou');
    });
  });
  ...
  ['setPanelPrice', 'setInverterPrice', 'setLaborPrice', 'setBatteryPrice', 'setEfficiency', 'setPanelWatt', 'setApiKey'].forEach(id => {
...
async function sendAIMessage() {
  ...
  // Load API Key from state/settings (DO NOT hardcode keys here for security)
  const API_KEY = state.settings.apiKey || '';
  const API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${API_KEY}`;

  if (!API_KEY) {
    loadingDiv.remove();
    addAIMessage('กรุณาใส่ API Key ในหน้าตั้งค่า (⚙️) ก่อนใช้งานนะครับ เนื่องจาก Key เดิมถูกระงับเพื่อความปลอดภัย', 'ai');
    return;
  }
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [{ text: `คุณคือ WiTSolarBot ข้อมูลปัจจุบัน: ระบบขนาด ${state.results ? state.results.kwp.toFixed(1) : 'ยังไม่ได้คำนวณ'} kWp. คำถามคือ: ${text}` }]
        }]
      })
    });
    ...
    if (response.ok && data.candidates && data.candidates[0].content.parts[0].text) {
      ...
      const errorMsg = data.error ? `${data.error.message} (${data.error.status})` : 'AI ปฏิเสธคำขอ';
      addAIMessage(`ขออภัยครับ: ${errorMsg}. ตรวจสอบ API Key ในหน้าตั้งค่าด้วยนะครับ`, 'ai');
    }
  } catch (e) {
    loadingDiv.remove();
    console.error("Fetch Error:", e);
    addAIMessage('ไม่สามารถเชื่อมต่อกับ AI ได้ ตรวจสอบ Internet หรือ API Key นะครับ', 'ai');
  }
}
...
function saveSettings() {
  const mapping = {
    'setPanelPrice': 'panelPrice',
    'setInverterPrice': 'inverterPrice',
    'setLaborPrice': 'laborPrice',
    'setBatteryPrice': 'batteryPrice',
    'setEfficiency': 'efficiency',
    'setPanelWatt': 'panelWatt',
    'setApiKey': 'apiKey'
  };
  ...
  Object.entries(mapping).forEach(([id, key]) => {
    const el = document.getElementById(id);
    if (el) {
      const val = (id === 'setApiKey') ? el.value : parseFloat(el.value);
...
function loadSettings() {
  ...
  if (saved) {
    try {
      ...
      Object.entries(mapping).forEach(([id, key]) => {
        const el = document.getElementById(id);
        if (el && state.settings[key] !== undefined) {
...
async function exportPDF() {
  ...
  const meterName = state.meter === 'tou' ? 'TOU Tariff' : 'Normal Tariff';
